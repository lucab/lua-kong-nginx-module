...
local kong_lua_kong_ffi_set_upstream_ssl_sans
...
if subsystem == "http" then
    ...
    int ngx_http_lua_kong_ffi_set_upstream_ssl_verify_depth(ngx_http_request_t *r,
        int depth);
    int ngx_http_lua_kong_ffi_set_upstream_ssl_sans(ngx_http_request_t *r,
    ...
    kong_lua_kong_ffi_set_upstream_ssl_sans = C.ngx_http_lua_kong_ffi_set_upstream_ssl_sans
...
elseif subsystem == 'stream' then
    ...
    int ngx_stream_lua_kong_ffi_set_upstream_ssl_verify_depth(ngx_stream_lua_request_t *r,
        int depth);
    int ngx_stream_lua_kong_ffi_set_upstream_ssl_sans(ngx_stream_lua_request_t *r,
        const char *input, size_t input_len);
    ...
    kong_lua_kong_ffi_set_upstream_ssl_sans = C.ngx_stream_lua_kong_ffi_set_upstream_ssl_sans
...
do
    ...
    function _M.set_upstream_ssl_sans(sans)
        if not ALLOWED_PHASES[get_phase()] then
            error("API disabled in the current context", 2)
        end
        ...
        local ret = kong_lua_kong_ffi_set_upstream_ssl_sans(r, ssl_sans, #ssl_sans)
        if ret == NGX_OK then
            return true
        end

        if ret == NGX_ERROR then
            return nil, "error while setting upstream ssl sans"
        end
